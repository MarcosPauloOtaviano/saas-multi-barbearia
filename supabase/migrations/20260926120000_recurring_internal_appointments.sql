-- Recurring appointments created by the barbershop team.
-- Each occurrence remains a normal appointment so the existing agenda,
-- automatic lifecycle and conflict constraints continue to work unchanged.
...
create index if not exists appointments_recurrence_idx
  on public.appointments (barbershop_id, recurrence_id, recurrence_position)
  ...
  while occurrence_date < end_date loop
    ...
    if should_create then
      ...
      insert into public.appointments (
        ...
        'pending', 'internal', nullif(trim(appointment_notes), ''),
