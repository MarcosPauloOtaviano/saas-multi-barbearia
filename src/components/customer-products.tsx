...
export function CustomerProducts({ slug }: { slug: string }) {
  const { shop, products, loading } = usePublicShop(slug);
  const activeProducts = products.filter((product) => product.active);
  return <section className="customer-page">
    <div className="customer-page-title"><p className="eyebrow">Produtos do estabelecimento</p><h1>Cuidados para levar.</h1><p>Itens disponíveis na {shop?.name ?? slug.split("-").join(" ")}, com preço real e compra combinada diretamente com a equipe.</p></div>
