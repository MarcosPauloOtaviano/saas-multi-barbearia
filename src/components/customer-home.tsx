...
export function CustomerHome({ slug, shop, shopName, services, barbers, products }: { slug: string; shop?: PublicShop | null; shopName?: string; services: Service[]; barbers: Barber[]; products: Product[] }) {
  ...
  return <div className="customer-home">
    ...
    {(shop?.address || shop?.phone || shop?.websiteUrl || shop?.instagramUrl || shop?.googleReviewsUrl) && <section className="customer-contact" aria-labelledby="customer-contact-title"><div><p className="eyebrow">Stilo Sampa em Guaxupé</p><h2 id="customer-contact-title">Visite ou fale com a equipe.</h2><p className="customer-contact__intro">Tudo o que você precisa para encontrar a barbearia e continuar seu atendimento pelos canais oficiais.</p></div><div className="customer-contact__links">{shop.address && <a href={mapHref ?? "#"} target="_blank" rel="noreferrer"><span><MapPin /></span><strong>{shop.address}</strong><ExternalLink /></a>}{shop.phone && <a href={`tel:${shop.phone.replace(/\D/g, "")}`}><span><Phone /></span><strong>{shop.phone}</strong><ArrowRight /></a>}{shop.instagramUrl && <a href={shop.instagramUrl} target="_blank" rel="noreferrer"><span><AtSign /></span><strong>Instagram da Stilo Sampa</strong><ExternalLink /></a>}{shop.websiteUrl && <a href={shop.websiteUrl} target="_blank" rel="noreferrer"><span><Scissors /></span><strong>Site oficial</strong><ExternalLink /></a>}{shop.googleReviewsUrl && <a href={shop.googleReviewsUrl} target="_blank" rel="noreferrer"><span><Star /></span><strong>{shop.googleReviewCount ? `${shop.googleReviewCount} avaliações no Google` : "Avaliações no Google"}</strong><ExternalLink /></a>}</div></section>}
