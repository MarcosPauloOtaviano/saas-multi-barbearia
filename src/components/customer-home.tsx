...
export function CustomerHome({ slug, shop, shopName, services, barbers }: { slug: string; shop?: PublicShop | null; shopName?: string; services: Service[]; barbers: Barber[] }) {
  ...
  return <div className="customer-home">
    <section className="customer-cinematic-hero cinematic-hero-shell" ref={heroRef} aria-labelledby="customer-professional-title">
      <div className="cinematic-hero">
        ...
        <div className="cinematic-hero__copy">
          ...
          <p className="cinematic-contact">{shop?.phone ? <>Fale com a equipe <strong>{shop.phone}</strong></> : <>Agendamento oficial de <strong>{name}</strong></>}</p>
        </div>
    ...
    <footer className="customer-footer"><div><Sparkles /><span><strong>{name}</strong><small>Agendamento oficial</small></span></div><div className="customer-footer__links"><Link href="/privacidade">Privacidade e LGPD</Link><Link href="/termos">Termos de uso</Link></div></footer>
  </div>;
}
