...
export function ServicesView() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await addService({ name: String(data.get("name")), description: String(data.get("description")), durationMinutes: Number(data.get("duration")), priceCents: Math.round(Number(data.get("price")) * 100) });
    setFeedback(result.message);
    if (result.ok) { form.reset(); window.setTimeout(() => { setOpen(false); setFeedback(""); }, 700); }
  }
  return <>
    ...
    {open && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}><section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="novo-servico"><div className="modal-header"><div><p className="eyebrow">Catálogo</p><h2 id="novo-servico">Novo serviço</h2></div><button className="icon-button" onClick={() => setOpen(false)} aria-label="Fechar"><X /></button></div><form className="form-grid" onSubmit={submit}><label className="field full"><span>Nome</span><input name="name" required minLength={2} /></label><label className="field full"><span>Descrição</span><textarea name="description" rows={3} required /></label><label className="field"><span>Duração (min)</span><input name="duration" type="number" min="10" step="5" defaultValue="45" required /></label><label className="field"><span>Preço (R$)</span><input name="price" type="number" min="0" step="0.01" defaultValue="45.00" required /></label>{feedback && <p className="form-feedback success full">{feedback}</p>}<div className="modal-actions full"><button type="button" className="button ghost" onClick={() => setOpen(false)}>Cancelar</button><button className="button primary">Cadastrar serviço</button></div></form></section></div>}
  </>;
}
