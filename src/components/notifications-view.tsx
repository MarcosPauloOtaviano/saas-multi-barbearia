  ...
  return <>
    ...
    <div className="notification-layout">
      <section className="content-card notification-feed">
        ...
        {notifications.map((note) => { const Icon = icons[note.type]; return <Link href={note.actionUrl} className={`notification-row ${!note.read ? "is-unread" : ""}`} onClick={() => markNotificationRead(note.id)} key={note.id}><span className={`notification-icon ${note.type}`}><Icon /></span><span><strong>{note.title}</strong><p>{note.body}</p><small>{note.time}</small></span>{!note.read && <i />}</Link>; })}
      </section>
