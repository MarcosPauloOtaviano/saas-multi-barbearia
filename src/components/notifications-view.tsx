...
export function NotificationsView() {
  const { notifications, markNotificationRead, markAllNotificationsRead, notificationsEnabled } = useAppData();
  ...
  return <>
    ...
    <div className="notification-layout">
      <section className="content-card notification-feed">
        ...
        {notifications.map((note) => { const Icon = icons[note.type]; const href = note.actionUrl.startsWith("/admin/") ? note.actionUrl : note.actionUrl.startsWith("/") ? `${base}${note.actionUrl}` : `${base}/notificacoes`; return <Link href={href} className={`notification-row ${!note.read ? "is-unread" : ""}`} onClick={() => markNotificationRead(note.id)} key={note.id}><span className={`notification-icon ${note.type}`}><Icon /></span><span><strong>{note.title}</strong><p>{note.body}</p><small>{note.time}</small></span>{!note.read && <i />}</Link>; })}
      </section>
      <aside className="content-card notification-summary"><span className="large-bell"><Bell /></span><h2>Você está em dia</h2><p>O assistente acompanha confirmações, cancelamentos, próximos horários e clientes prontos para retornar.</p><div><span><strong>Avisos neste aparelho</strong><small>Som e notificação enquanto o painel estiver aberto</small></span><b className={notificationsEnabled ? "" : "neutral"}>{notificationsEnabled ? "Ativo" : "Desativado"}</b></div><div><span><strong>E-mail</strong><small>Confirmações e lembretes dependem do provedor</small></span><b className="neutral">Configurar</b></div><div><span><strong>WhatsApp do cliente</strong><small>Número obrigatório no agendamento para contato rápido</small></span><b className="neutral">Disponível</b></div></aside>
    </div>
  </>;
}
