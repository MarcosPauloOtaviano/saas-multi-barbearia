  ...
  async function finish(event: FormEvent<HTMLFormElement>) {
    ...
    if (hasSupabaseEnv && supabaseUrl && supabasePublishableKey) {
      ...
      if (!response.ok) {
        ...
        setBookingError(errorPayload.error === "whatsapp_required" ? "Informe um número de WhatsApp válido para receber avisos do atendimento." : response.status === 409 ? "Esse horário acabou de ser reservado ou conflita com outro agendamento seu. Escolha outro." : "Não foi possível concluir o agendamento.");
        return;
      }
