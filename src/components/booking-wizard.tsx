...
export function BookingWizard({ slug, initialServiceId = "" }: { slug: string; initialServiceId?: string }) {
  ...
  return <main className="booking-page">
    ...
    <section className="booking-card">
      ...
      {step === 5 && <div className="booking-success"><span><CheckCircle2 /></span><p className="eyebrow">Tudo certo</p><h2>Agendamento confirmado!</h2>{emailDelivered ? <p>Enviamos os detalhes para <strong>{customer.email}</strong>. Você poderá confirmar ou cancelar pelo link seguro no e-mail — sem senha e sem criar conta.</p> : <p>Seu horário está reservado, mas o e-mail não pôde ser enviado agora. Guarde esta tela e, se precisar, fale com a barbearia para confirmar os detalhes.</p>}<div className="success-ticket"><div><small>Serviço</small><strong>{selectedService?.name}</strong></div><div><small>Profissional</small><strong>{selectedBarber?.name}</strong></div><div><small>Data e horário</small><strong>{bookingDates.find((day) => day.iso === selectedDate)?.day} · {time}</strong></div></div><button className="button subtle" onClick={() => { requestId.current = null; setStep(1); setServiceId(""); setBarberId(""); setAssignedBarberId(""); setTime(""); setSlotIso(""); setEmailDelivered(true); }}>Fazer outro agendamento</button></div>}
    </section>
