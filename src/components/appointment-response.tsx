...
export function AppointmentResponse() {
  ...
  return <main className="response-page"><div className="response-brand"><span><Scissors /></span> Stilo Sampa</div><section className="response-card">{state === "loading" && <><LoaderCircle className="spin" /><h1>Atualizando seu agendamento…</h1><p>Isso leva apenas alguns segundos.</p></>}{state === "success" && <>{action === "confirm" ? <CalendarCheck className="response-success" /> : <CircleX className="response-cancel" />}<p className="eyebrow">Resposta registrada</p><h1>{action === "confirm" ? "Presença confirmada!" : "Agendamento cancelado"}</h1><p>{action === "confirm" ? "A barbearia já foi avisada. Esperamos você no horário combinado." : "O horário foi liberado e a barbearia já recebeu a atualização."}</p><Link className="button primary" href="/b/stilo-sampa">Voltar à página da barbearia</Link></>}{state === "error" && <><CircleX className="response-cancel" /><p className="eyebrow">Link indisponível</p><h1>Não foi possível atualizar</h1><p>O link pode ter expirado ou já ter sido utilizado. Entre em contato com a barbearia.</p></>}</section></main>;
}
